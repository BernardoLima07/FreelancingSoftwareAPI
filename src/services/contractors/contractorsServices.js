...
export class ContractorsService {
    ...
    async listContractors({ limit = 10, attributes = ['id', 'name', 'email', 'balance'], whereType = {} }) {
        return this.contractorsRepository.findAll({ limit: limit, attributes: attributes, whereType: whereType })
    }
    ...
    async registerContractor({ name, email, password, balance = 0 }) {
        ...
        const existingContractor = await this.contractorsRepository.findOne({ email })

        if (existingContractor) throw new Error('Email already registered')

        return this.contractorsRepository.register({ name: name, email: email, passwordHash: passwordHash, balance: balance })
    }
    ...
    async updateContractor({ contractorId, updatedData }) {
        return await this.contractorsRepository.update({ contractor_id: contractorId, updatedData: updatedData })
    }
