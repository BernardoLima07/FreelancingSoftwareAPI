...
export class JobsService {
    ...
    async listJobs({ limit = 10, attributes, order, whereType }) {
        return await this.jobRepository.findAll({ limit: limit, attributes: attributes, order: order, whereType: whereType })
    }
    ...
    async update({jobId, updatedData}) {
        return await this.jobRepository.update({ jobId: jobId, updateData: updatedData })
    }
}